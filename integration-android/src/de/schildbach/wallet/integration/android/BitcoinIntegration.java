...
public final class BitcoinIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/smileycoin-paymentrequest"; // BIP 71
	...
	{
		final StringBuilder uri = new StringBuilder("smileycoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No smileycoin application found.\nPlease install smileycoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=hashengineering.smileycoin.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/HashEngineering/smileycoin-wallet/releases"));
